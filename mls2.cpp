	...
	typename pcl::PointCloud<PointT>::Ptr deform(){
		...
		for(int n=0;n<cloudSize;n++){
			...
			for(int i=0;i<controlSize;i++){
				PQt += weights(n,i)*P.col(i)*(Q.col(i).transpose());
				Pstar+=weights(n,i)*P.col(i);
				Qstar+=weights(n,i)*Q.col(i);
			}
			const float weightSum = weights.row(n).sum();
			Pstar/=weightSum;
			Qstar/=weightSum;
...
int main(int argv,char *args[]){
	...
	Ptr cloud = get<0>(temp);
	Ptr center = get<1>(temp);
	Ptr rim = get<2>(temp);
